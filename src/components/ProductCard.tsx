...
const PLATFORM_STYLE = {
  번개장터: { bg: 'bg-red-500', label: 'B' },
  중고나라: { bg: 'bg-teal-500', label: 'J' },
} as const;
...
export default function ProductCard({ product, size = 'large' }: Props) {
  const { platform, title, price, link, update_time, image, status } = product;
  ...
  return (
    <a
      ...
      className="group block bg-white border border-gray-100 rounded-xl overflow-hidden hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200"
    >
      {/* 이미지 영역 - 1:1 비율 */}
      <div className="relative aspect-square bg-gray-50 overflow-hidden">
        ...
        {/* 플랫폼 뱃지 */}
        <span
          className={`absolute top-2 left-2 ${badge.bg} text-white text-[10px] font-bold w-5 h-5 flex items-center justify-center rounded-full shadow-sm`}
        >
          {badge.label}
        </span>
        {/* 판매완료 오버레이 */}
      ...
      {/* 정보 영역 */}
      <div className={`p-3 ${size === 'small' ? 'p-2' : ''}`}>
        <p
          className={`text-gray-800 font-medium leading-snug mb-1.5 line-clamp-2 ${
        ...
        <p className="text-gray-400 text-[11px] mt-1">{update_time} · {platform}</p>
      </div>
    </a>
  );
}
